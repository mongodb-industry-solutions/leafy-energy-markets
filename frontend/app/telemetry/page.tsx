...
import type { TelemetryMode } from '@/components/telemetry/ControlPanel';
...
const GENERATOR_CHART_META = SUBSTATIONS.map((s) => ({
  id: s.id,
  name: s.name.split(' ').slice(0, 2).join(' '),
  color: GENERATOR_COLORS[s.id],
}));

// ── Output simulation per fuel type ──────────────────────
...
// ── Originator events (for event feed) ───────────────────
...
export default function TelemetryPage() {
  ...
  const appendMetrics = useCallback((metrics: TelemetryMetrics, elapsed: number) => {
    ...
    // Generate originator events for the feed
    ...
    // Update per-substation readings
    ...
    // Build generator output chart point
  ...
  const handleStart = useCallback(async () => {
    ...
    if (mode === 'simulation') {
      ...
      // Backend mode — try real API, fall back to simulation
      try {
  ...
  }, [config, mode, startRealStream, startSimulatedStream]);
